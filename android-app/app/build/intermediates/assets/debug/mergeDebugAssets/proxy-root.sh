#!/system/bin/sh
# Hetu Root transparent proxy controller v2. JSON only on stdout.
set -u
umask 077
...
# Android netd uses its own fwmark fields. Hetu only considers the AOSP-reserved
...
probetp4(){
  P="$1"
  iptables -t mangle -N HETU_PROBE >/dev/null 2>&1 || true
  iptables -t mangle -F HETU_PROBE >/dev/null 2>&1 || true
  iptables -t mangle -A HETU_PROBE -p udp -j TPROXY --on-port "$P" --tproxy-mark "$PROBE_MARK/$PROBE_MASK" >/dev/null 2>&1
  R=$?
  iptables -t mangle -F HETU_PROBE >/dev/null 2>&1 || true
  iptables -t mangle -X HETU_PROBE >/dev/null 2>&1 || true
  return "$R"
}

probetp6(){
  P="$1"; has ip6tables || return 1
  ip6tables -t mangle -N HETU_PROBE >/dev/null 2>&1 || true
  ip6tables -t mangle -F HETU_PROBE >/dev/null 2>&1 || true
  ip6tables -t mangle -A HETU_PROBE -p udp -j TPROXY --on-port "$P" --tproxy-mark "$PROBE_MARK/$PROBE_MASK" >/dev/null 2>&1
  R=$?
  ip6tables -t mangle -F HETU_PROBE >/dev/null 2>&1 || true
  ip6tables -t mangle -X HETU_PROBE >/dev/null 2>&1 || true
  return "$R"
}

probered4(){
  P="$1"; PROTO="${2:-tcp}"
  iptables -t nat -N HETU_PROBE >/dev/null 2>&1 || true
  iptables -t nat -F HETU_PROBE >/dev/null 2>&1 || true
  iptables -t nat -A HETU_PROBE -p "$PROTO" -j REDIRECT --to-ports "$P" >/dev/null 2>&1
  R=$?
  iptables -t nat -F HETU_PROBE >/dev/null 2>&1 || true
  iptables -t nat -X HETU_PROBE >/dev/null 2>&1 || true
  return "$R"
}

probered6(){
  P="$1"; PROTO="${2:-tcp}"; has ip6tables || return 1
  ip6tables -t nat -N HETU_PROBE >/dev/null 2>&1 || true
  ip6tables -t nat -F HETU_PROBE >/dev/null 2>&1 || true
  ip6tables -t nat -A HETU_PROBE -p "$PROTO" -j REDIRECT --to-ports "$P" >/dev/null 2>&1
  R=$?
  ip6tables -t nat -F HETU_PROBE >/dev/null 2>&1 || true
  ip6tables -t nat -X HETU_PROBE >/dev/null 2>&1 || true
  return "$R"
}
...
status(){
  ...
  printf '{"ok":true,"running":%s,"pid":%s,"mode":"%s","ipv4Rules":%s,"ipv6Rules":%s,"ipv6DisabledByHetu":%s,"watchdog":%s,"recoveredStaleRules":%s,"mark":"%s","table":"%s","log":"%s","configCheckLog":"%s"}\n' \
