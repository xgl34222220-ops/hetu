...
public final class BichenApplication extends Application implements Application.ActivityLifecycleCallbacks {
    ...
    @Override public void onActivityCreated(Activity a,Bundle b){if(a instanceof MainActivity){SkinSession s=new SkinSession(a);sessions.put(a,s);a.getWindow().getDecorView().post(s::install);}else if(a instanceof ProxyActivity){a.getWindow().getDecorView().post(()->floatProxyDock(a));}}
    ...
    private static void floatProxyDock(Activity a){
        try{Field nf=ProxyActivity.class.getDeclaredField("nav");nf.setAccessible(true);Object no=nf.get(a);if(!(no instanceof LuoShuDockView))return;LuoShuDockView nav=(LuoShuDockView)no;ViewParent vp=nav.getParent();if(!(vp instanceof ViewGroup))return;((ViewGroup)vp).removeView(nav);View hostView=a.findViewById(android.R.id.content);if(!(hostView instanceof FrameLayout))return;FrameLayout host=(FrameLayout)hostView;FrameLayout.LayoutParams lp=new FrameLayout.LayoutParams(-1,dp(a,72),Gravity.BOTTOM);lp.leftMargin=dp(a,20);lp.rightMargin=dp(a,20);lp.bottomMargin=dp(a,12);host.addView(nav,lp);nav.setElevation(dp(a,18));nav.setOnApplyWindowInsetsListener((v,insets)->{int bottom=Build.VERSION.SDK_INT>=30?insets.getInsets(WindowInsets.Type.navigationBars()).bottom:insets.getSystemWindowInsetBottom();ViewGroup.LayoutParams raw=v.getLayoutParams();if(raw instanceof FrameLayout.LayoutParams){FrameLayout.LayoutParams p=(FrameLayout.LayoutParams)raw;int wanted=dp(a,12)+bottom;if(p.bottomMargin!=wanted){p.bottomMargin=wanted;v.setLayoutParams(p);}}return insets;});nav.requestApplyInsets();Field cf=ProxyActivity.class.getDeclaredField("content");cf.setAccessible(true);Object co=cf.get(a);if(co instanceof FrameLayout){FrameLayout content=(FrameLayout)co;content.setClipToPadding(false);content.setPadding(content.getPaddingLeft(),content.getPaddingTop(),content.getPaddingRight(),dp(a,92));}}catch(Throwable ignored){}
    }
    ...
    private static final class SkinSession implements ViewTreeObserver.OnGlobalLayoutListener {
        ...
        final WeakHashMap<View,Boolean> styled=new WeakHashMap<>();final WeakHashMap<View,Boolean> rootStyled=new WeakHashMap<>();boolean scheduled,disposed;LinearLayout mainDock,lastDockGeometry;LuoShuDockView overlayDock;int lastDockTab=-1;
        SkinSession(Activity a){this.a=a;dark=ProxyUi.isDark(a);int dyn;if(Build.VERSION.SDK_INT>=31)dyn=a.getColor(dark?android.R.color.system_accent1_200:android.R.color.system_accent1_500);else dyn=dark?0xffa9bfff:0xff4f6fd8;bg=dark?0xff111214:mix(0xfff4f6fa,dyn,.055f);surface=dark?0xff1b1c20:mix(0xffffffff,dyn,.012f);text=dark?0xfff1f2f5:0xff16171b;muted=dark?0xffa8abb4:0xff70727c;accent=dyn;soft=mix(surface,accent,dark?.13f:.09f);danger=dark?0xffffb39d:0xffa64d3d;divider=dark?0x24ffffff:0x10000000;}
        ...
        private void styleKnownRoots(){
            styleRoot("shell",v->v.setBackground(new LuoShuSurfaceDrawable(bg,accent,Color.TRANSPARENT,0,dark?.045f:.065f,0,true)));
            styleRoot("banner",v->{if(v instanceof LinearLayout){v.setBackground(new LuoShuSurfaceDrawable(soft,accent,dark?0x18ffffff:0x42ffffff,dp(16),.02f,dark?.035f:.11f,false));v.setPadding(dp(13),dp(8),dp(7),dp(8));}});
            styleRoot("heading",v->{if(v instanceof TextView){TextView t=(TextView)v;t.setTextColor(text);t.setTextSize(TypedValue.COMPLEX_UNIT_SP,29);t.setTypeface(Typeface.create("sans-serif-black",Typeface.NORMAL));ViewParent labels=t.getParent();if(labels instanceof View&&((View)labels).getParent() instanceof LinearLayout){LinearLayout header=(LinearLayout)((View)labels).getParent();header.setPadding(dp(20),dp(11),dp(18),dp(11));}}});
            styleRoot("subtitle",v->{if(v instanceof TextView){TextView t=(TextView)v;t.setTextColor(muted);t.setTextSize(TypedValue.COMPLEX_UNIT_SP,11.5f);t.setLineSpacing(0,1);}});
        }
        ...
        private void styleMainDock(){try{Field f=MainActivity.class.getDeclaredField("nav");f.setAccessible(true);Object o=f.get(a);if(!(o instanceof LinearLayout))return;mainDock=(LinearLayout)o;boolean newDock=mainDock!=lastDockGeometry;if(newDock){lastDockGeometry=mainDock;lastDockTab=-1;mainDock.setAlpha(0f);mainDock.setImportantForAccessibility(View.IMPORTANT_FOR_ACCESSIBILITY_NO_HIDE_DESCENDANTS);ViewGroup.LayoutParams old=mainDock.getLayoutParams();if(old instanceof LinearLayout.LayoutParams){LinearLayout.LayoutParams gone=(LinearLayout.LayoutParams)old;gone.height=0;gone.setMargins(0,0,0,0);mainDock.setLayoutParams(gone);}if(overlayDock!=null&&overlayDock.getParent() instanceof ViewGroup)((ViewGroup)overlayDock.getParent()).removeView(overlayDock);View hostView=a.findViewById(android.R.id.content);if(!(hostView instanceof FrameLayout))return;FrameLayout host=(FrameLayout)hostView;overlayDock=new LuoShuDockView(a,new String[]{"首页","应用","规则","活动"},new String[]{"shield","apps","rules","activity"},currentTab(),accent,muted,surface,dark,index->{try{if(index<mainDock.getChildCount())mainDock.getChildAt(index).performClick();}catch(Throwable ignored){}});FrameLayout.LayoutParams lp=new FrameLayout.LayoutParams(-1,dp(72),Gravity.BOTTOM);lp.leftMargin=dp(20);lp.rightMargin=dp(20);lp.bottomMargin=dp(12);host.addView(overlayDock,lp);overlayDock.setOnApplyWindowInsetsListener((v,insets)->{int bottom=Build.VERSION.SDK_INT>=30?insets.getInsets(WindowInsets.Type.navigationBars()).bottom:insets.getSystemWindowInsetBottom();ViewGroup.LayoutParams raw=v.getLayoutParams();if(raw instanceof FrameLayout.LayoutParams){FrameLayout.LayoutParams p=(FrameLayout.LayoutParams)raw;int wanted=dp(12)+bottom;if(p.bottomMargin!=wanted){p.bottomMargin=wanted;v.setLayoutParams(p);}}return insets;});overlayDock.requestApplyInsets();}}catch(Throwable ignored){}}
        ...
        private void styleOne(View v){
            ...
            if(v instanceof TextView){TextView t=(TextView)v;int c=t.getCurrentTextColor();if(isOldText(c))t.setTextColor(text);else if(isOldMuted(c))t.setTextColor(muted);else if(isOldAccent(c))t.setTextColor(accent);else if(isOldDanger(c))t.setTextColor(danger);t.setLineSpacing(0,1.03f);if(v instanceof EditText){t.setBackground(new LuoShuSurfaceDrawable(surface,accent,dark?0x14ffffff:0x34ffffff,dp(17),.012f,dark?.025f:.075f,false));t.setPadding(dp(15),dp(10),dp(15),dp(10));t.setMinHeight(dp(46));}else if(t.isClickable()&&Color.alpha(t.getCurrentTextColor())>0&&isNearlyWhite(t.getCurrentTextColor())){t.setBackground(round(accent,18));t.setMinHeight(dp(48));}CharSequence value=t.getText();if(value!=null&&value.length()>80&&!t.isTextSelectable()){t.setMaxLines(3);t.setEllipsize(TextUtils.TruncateAt.END);}return;}
            ...
            if(v instanceof LinearLayout){LinearLayout l=(LinearLayout)v;if(l.getElevation()>.5f){boolean hero=containsIcon(l,"power"),warning=containsText(l,"需要处理"),proxy=containsText(l,"代理与去广告 · Mihomo"),appIntro=containsText(l,"选中的应用直接放行");int radius=hero?28:warning?18:22;int base=warning?soft:hero?mix(surface,accent,dark?.11f:.085f):surface;l.setBackground(new LuoShuSurfaceDrawable(base,accent,dark?0x14ffffff:0x30ffffff,dp(radius),hero?(dark?.075f:.095f):.012f,hero?(dark?.08f:.18f):(dark?.03f:.075f),false));l.setElevation(dp(hero?6:warning?0:1));int px=hero?18:warning||proxy?14:16,py=hero?18:warning||proxy?11:14;l.setPadding(dp(px),dp(py),dp(px),dp(py));if(appIntro)l.setPadding(dp(16),dp(14),dp(16),dp(14));if(warning)compactLongText(l,2);}}
            if(v instanceof FrameLayout&&v.getElevation()>.5f){v.setBackground(new LuoShuSurfaceDrawable(surface,accent,dark?0x14ffffff:0x34ffffff,dp(16),.018f,dark?.04f:.10f,false));}
            if(v instanceof ListView){ListView l=(ListView)v;l.setBackgroundColor(Color.TRANSPARENT);l.setClipToPadding(false);}
            if(v instanceof ScrollView){ScrollView s=(ScrollView)v;s.setClipToPadding(false);s.setPadding(s.getPaddingLeft(),s.getPaddingTop(),s.getPaddingRight(),Math.max(s.getPaddingBottom(),dp(96)));}
        }
