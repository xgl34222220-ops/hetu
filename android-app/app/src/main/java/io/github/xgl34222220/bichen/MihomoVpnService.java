...
public final class MihomoVpnService extends VpnService {
 ...
 @Override public int onStartCommand(Intent intent,int flags,int startId){
  ...
  if("STOP".equals(action)){stopping=true;prefs.edit().putBoolean("proxyWanted",false).apply();worker.execute(this::finishStop);return START_NOT_STICKY;}
  if(engaged)return START_NOT_STICKY;engaged=true;stopping=false;failure="";state="正在校验配置";foreground();
  prefs.edit().putBoolean("proxyWanted",true).putString("engineOwner","mihomo").apply();
  worker.execute(()->{try{startCore();}catch(Exception|LinkageError e){failure=e.getMessage()==null?"内核启动失败":e.getMessage();finishStop();}});return START_NOT_STICKY;
 }
 private void startCore()throws Exception{
  ...
  // Core outbounds share this excluded UID and use the physical network.
  // No allowBypass or silent VPN fallback is enabled.
 ...
 private void finishStop(){
  ...
  if(prefs.getBoolean("proxyRestoreHosts",false)){try{
   ...
   prefs.edit().putBoolean("proxyRestoreHosts",false).commit();
  }catch(Exception e){failure="代理已停止；原模块恢复未完成，请到保护页检查";}}
  prefs.edit().putBoolean("proxyWanted",false).putString("proxyError",failure).apply();engaged=false;state=failure.isEmpty()?"已停止":failure;stopForeground(true);stopSelf();
 }
 ...
 @Override public void onRevoke(){stopping=true;prefs.edit().putBoolean("proxyWanted",false).apply();worker.execute(this::finishStop);}
 @Override public void onDestroy(){stopping=true;closeDescriptor();if(engaged)worker.execute(this::finishStop);worker.shutdown();super.onDestroy();}
}
