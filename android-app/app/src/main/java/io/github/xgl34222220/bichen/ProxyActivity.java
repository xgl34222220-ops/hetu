...
public final class ProxyActivity extends Activity {
 ...
 private void build(){
  ...
  button(c,"重试恢复原模块",()->{if(!MihomoVpnService.engaged)startService(new Intent(this,MihomoVpnService.class).setAction("RESTORE"));});
 ...
 @Override protected void onActivityResult(int request,int result,Intent data){super.onActivityResult(request,result,data);if(request==902){if(result==RESULT_OK)launch();else toast("未授权 VPN，配置没有改变");}else if(request==901&&result==RESULT_OK&&data!=null&&!locked()){task(()->{String yaml=ProxyStore.read(getContentResolver().openInputStream(data.getData()));store.save(yaml,"");return "已导入原始 YAML，未上传或覆盖原文件";});}}
