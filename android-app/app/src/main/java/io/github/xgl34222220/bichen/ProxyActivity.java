...
public final class ProxyActivity extends Activity {
    ...
    private LinearLayout nav;private FrameLayout content;private TextView title,subtitle,notice,status,detail,power,downMetric,upMetric,countMetric,strategySummary;
    private ListView list;private BaseAdapter adapter;private int page,panelTab;private boolean busy,running,resumed,closed,polling;private String nodeSearch="",message="";
    ...
    private void build(){
        ...
        nav=u.row();nav.setPadding(u.dp(6),u.dp(6),u.dp(6),u.dp(6));nav.setBackground(u.glassDock());if(Build.VERSION.SDK_INT>=21)nav.setElevation(u.dp(14));LinearLayout.LayoutParams bp=new LinearLayout.LayoutParams(-1,u.dp(70));bp.setMargins(u.dp(20),u.dp(5),u.dp(20),u.dp(12));shell.addView(nav,bp);
    ...
    private void renderNav(){
        ...
        for(int i=0;i<4;i++){final int n=i;LinearLayout item=u.col();item.setGravity(Gravity.CENTER);item.setBackground(u.touch(page==i?u.soft:Color.TRANSPARENT,23));item.addView(new IconView(this,icons[i],page==i?u.accent:u.muted),new LinearLayout.LayoutParams(u.dp(22),u.dp(22)));u.gap(item,4);TextView t=u.text(names[i],11,page==i?u.accent:u.muted,page==i);t.setGravity(Gravity.CENTER);item.addView(t);item.setOnClickListener(v->showPage(n));nav.addView(item,new LinearLayout.LayoutParams(0,-1,1));}
    }
    ...
    private void panelTabs(LinearLayout p){LinearLayout tabs=u.row();tabs.setPadding(0,0,0,u.dp(8));tabs.addView(u.chip("策略组",panelTab==0,()->{panelTab=0;showPage(1);}),new LinearLayout.LayoutParams(0,-2,1));tabs.addView(u.chip("连接",panelTab==1,()->{panelTab=1;showPage(1);}),new LinearLayout.LayoutParams(0,-2,1));p.addView(tabs);}
    ...
    private LinearLayout rowCard(){LinearLayout c=u.col();c.setPadding(u.dp(18),u.dp(16),u.dp(18),u.dp(16));c.setBackground(u.bg(u.surface,24));if(Build.VERSION.SDK_INT>=21)c.setElevation(u.dp(1));return c;}
    ...
    private void tools(LinearLayout body){LinearLayout runtime=u.card(body);runtime.addView(u.text("运行工具",17,u.text,true));u.gap(runtime,4);u.action(runtime,"activity","运行日志","核心启动 · Root · 透明代理规则",this::showLog);u.separator(runtime);u.action(runtime,"rules","最终启动配置","查看实际交给核心的 startup-config",this::showStartup);LinearLayout manage=u.card(body);manage.addView(u.text("管理",17,u.text,true));u.gap(manage,4);u.action(manage,"folder","核心与配置","核心导入 · 配置选择 · 自动覆写",()->startActivity(new Intent(this,RootTproxyActivity.class)));u.separator(manage);u.action(manage,"globe","策略组与节点","节点切换 · 单节点/整组测速",()->{panelTab=0;showPage(1);});u.separator(manage);u.action(manage,"activity","实时连接","目标 · 规则 · 代理链 · 流量",()->{panelTab=1;showPage(1);});}
    ...
    private void settings(LinearLayout body){ProxyRuntimeProfile p=profile();ProxyConfigLibrary.Entry e=configs.selected(p.core);LinearLayout current=u.card(body);current.addView(u.text("基础代理配置",17,u.text,true));u.gap(current,8);current.addView(u.text(p.summary()+"\n"+(e==null?"未选择配置":e.name),12,u.muted,false));u.gap(current,4);u.action(current,"settings","进入基础代理配置","核心 · 运行模式 · IPv6 · 自动覆写 · 配置选择",()->startActivity(new Intent(this,RootTproxyActivity.class)));LinearLayout note=u.card(body);note.addView(u.text("功能架构按 BoxProxy，视觉统一洛书",14,u.text,true));u.gap(note,7);note.addView(u.text("策略组、节点、测速、连接和 Root 透明代理保持真实后端；页面、卡片、悬浮底栏与交互风格统一到洛书。",12,u.muted,false));}
    ...
    private void poll(){if(polling||closed)return;polling=true;worker.execute(()->{boolean live=false;JSONObject g=new JSONObject(),c=new JSONObject();String error="";try{live=root.status().optBoolean("running",false);if(live){try{g=controller.proxies();c=controller.connections();}catch(Exception api){error=safe(api);}}}catch(Exception e){error=safe(e);}boolean r=live;JSONObject gg=g,cc=c;String err=error;ui.post(()->{polling=false;if(closed)return;running=r;groups=gg;connections=cc;if(!err.isEmpty()&&running)setMessage("代理运行中 · 面板暂未就绪："+err);else if(running&&message.startsWith("代理运行中 · 面板"))setMessage("");updateVisible();notifyData();});});}
    ...
    private String strategyText(){if(!running)return"启动代理后可选择策略组和节点";List<String> names=groupNames();if(names.isEmpty())return"面板正在初始化，或当前配置没有可显示的策略组";StringBuilder b=new StringBuilder();for(int i=0;i<Math.min(3,names.size());i++){JSONObject g=groups.optJSONObject(names.get(i));if(i>0)b.append('\n');b.append(names.get(i)).append("  →  ").append(g==null?"—":g.optString("now","—"));}if(names.size()>3)b.append("\n还有 ").append(names.size()-3).append(" 个策略组");return b.toString();}
