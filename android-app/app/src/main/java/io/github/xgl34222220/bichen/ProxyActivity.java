...
/** BoxProxy-style Root console: home, panel, tools and settings. */
public final class ProxyActivity extends Activity {
    ...
    private MihomoControllerClient controller;
    private LinearLayout nav;
    private FrameLayout content;
    private TextView title,subtitle,notice,status,detail,power,downMetric,upMetric,countMetric,strategySummary;
    private ListView list;
    private BaseAdapter adapter;
    private int page,panelTab;
    private boolean busy,running,resumed,closed,polling;
    private String nodeSearch="",message="";
    ...
    @Override public void onCreate(Bundle state){
        prefs=getSharedPreferences("bichen",0);
        setTheme(ProxyUi.isDark(this)?R.style.AppThemeDark:R.style.AppTheme);
        super.onCreate(state);
        u=new ProxyUi(this);root=new RootProxyManager(this);configs=new ProxyConfigLibrary(this);controller=new MihomoControllerClient(this);
        if(state!=null){page=state.getInt("page",0);panelTab=state.getInt("panelTab",0);nodeSearch=state.getString("search","");}
        build();showPage(page);
    }
    ...
    private void build(){
        ...
        LinearLayout head=u.row();head.setPadding(u.dp(18),u.dp(10),u.dp(18),u.dp(9));
        head.addView(u.icon("back","返回",this::finish),new LinearLayout.LayoutParams(u.dp(48),u.dp(48)));
        LinearLayout labels=u.col();labels.setPadding(u.dp(12),0,0,0);title=u.text("代理",29,u.text,true);labels.addView(title);u.gap(labels,4);subtitle=u.text("Root 服务与透明代理",12,u.muted,false);labels.addView(subtitle);head.addView(labels,new LinearLayout.LayoutParams(0,-2,1));shell.addView(head);
        notice=u.text("",12,u.accent,false);notice.setPadding(u.dp(14),u.dp(10),u.dp(14),u.dp(10));notice.setBackground(u.bg(u.soft,16));notice.setMaxLines(2);notice.setEllipsize(TextUtils.TruncateAt.END);notice.setVisibility(View.GONE);notice.setOnClickListener(v->new AlertDialog.Builder(this).setTitle("状态详情").setMessage(message).setPositiveButton("关闭",null).show());LinearLayout.LayoutParams np=new LinearLayout.LayoutParams(-1,-2);np.setMargins(u.dp(18),0,u.dp(18),u.dp(8));shell.addView(notice,np);
        content=new FrameLayout(this);shell.addView(content,new LinearLayout.LayoutParams(-1,0,1));
        nav=u.row();nav.setPadding(u.dp(5),u.dp(5),u.dp(5),u.dp(5));GradientDrawable glass=new GradientDrawable(GradientDrawable.Orientation.TOP_BOTTOM,new int[]{u.surface,u.dark?0xff213027:0xffedf3ef});glass.setCornerRadius(u.dp(27));nav.setBackground(glass);nav.setElevation(u.dp(7));LinearLayout.LayoutParams bp=new LinearLayout.LayoutParams(-1,u.dp(68));bp.setMargins(u.dp(18),u.dp(5),u.dp(18),u.dp(10));shell.addView(nav,bp);
        setContentView(shell);shell.requestApplyInsets();renderNav();
    }

    private void renderNav(){
        nav.removeAllViews();String[] names={"首页","面板","工具","设置"},icons={"shield","globe","apps","settings"};
        for(int i=0;i<4;i++){final int n=i;LinearLayout item=u.col();item.setGravity(Gravity.CENTER);item.setBackground(u.touch(page==i?u.soft:Color.TRANSPARENT,21));item.addView(new IconView(this,icons[i],page==i?u.accent:u.muted),new LinearLayout.LayoutParams(u.dp(22),u.dp(22)));u.gap(item,4);TextView t=u.text(names[i],10.5f,page==i?u.accent:u.muted,page==i);t.setGravity(Gravity.CENTER);item.addView(t);item.setOnClickListener(v->showPage(n));nav.addView(item,new LinearLayout.LayoutParams(0,-1,1));}
    }
    ...
    private void showPage(int n){
        ...
        else{ScrollView s=new ScrollView(this);s.setVerticalScrollBarEnabled(false);LinearLayout body=u.col();body.setPadding(u.dp(18),u.dp(3),u.dp(18),u.dp(12));s.addView(body);content.addView(s,new FrameLayout.LayoutParams(-1,-1));if(page==0)home(body);else if(page==2)tools(body);else settings(body);}
        renderNav();updateVisible();poll();
    }

    private TextView metric(LinearLayout row,String name){LinearLayout box=u.col();TextView v=u.text("—",18,u.text,true);box.addView(v);u.gap(box,4);box.addView(u.text(name,10.5f,u.muted,false));row.addView(box,new LinearLayout.LayoutParams(0,-2,1));return v;}

    private void home(LinearLayout body){
        LinearLayout hero=u.card(body);GradientDrawable g=new GradientDrawable(GradientDrawable.Orientation.TL_BR,new int[]{u.dark?0xff203b31:0xffe1f0e7,u.dark?0xff17271f:0xfff5f1e8});g.setCornerRadius(u.dp(26));hero.setBackground(g);
        LinearLayout top=u.row();LinearLayout left=u.col();status=u.text("正在读取",25,u.text,true);left.addView(status);u.gap(left,7);detail=u.text(profileLine(),12,u.muted,false);left.addView(detail);top.addView(left,new LinearLayout.LayoutParams(0,-2,1));power=u.button("启动",true,this::toggle);top.addView(power,new LinearLayout.LayoutParams(u.dp(90),u.dp(54)));hero.addView(top);u.gap(hero,22);LinearLayout metrics=u.row();downMetric=metric(metrics,"下载");upMetric=metric(metrics,"上传");countMetric=metric(metrics,"连接");hero.addView(metrics);
        LinearLayout policy=u.card(body);LinearLayout h=u.row();h.addView(u.text("当前策略",16,u.text,true),new LinearLayout.LayoutParams(0,-2,1));h.addView(u.chip("打开面板",false,()->{panelTab=0;showPage(1);}));policy.addView(h);u.gap(policy,9);strategySummary=u.text("启动代理后读取实际策略组",12.5f,u.muted,false);strategySummary.setLineSpacing(u.dp(4),1);policy.addView(strategySummary);
        LinearLayout quick=u.card(body);u.action(quick,"globe","节点选择","策略组 · 节点切换 · 单节点/整组测速",()->{panelTab=0;showPage(1);});u.action(quick,"activity","连接活动","目标 · 规则 · 代理链 · 流量",()->{panelTab=1;showPage(1);});u.action(quick,"folder","基础代理配置",profile().summary(),()->startActivity(new Intent(this,RootTproxyActivity.class)));
    }

    private void panelTabs(LinearLayout p){LinearLayout tabs=u.row();tabs.setPadding(0,0,0,u.dp(8));tabs.setBackground(u.bg(u.surface,18));tabs.addView(u.chip("策略组",panelTab==0,()->{panelTab=0;showPage(1);}),new LinearLayout.LayoutParams(0,-2,1));tabs.addView(u.chip("连接",panelTab==1,()->{panelTab=1;showPage(1);}),new LinearLayout.LayoutParams(0,-2,1));p.addView(tabs);}
    private LinearLayout listShell(){LinearLayout p=u.col();p.setPadding(u.dp(18),u.dp(2),u.dp(18),0);content.addView(p,new FrameLayout.LayoutParams(-1,-1));return p;}
    private void addList(LinearLayout p,BaseAdapter a){list=new ListView(this);list.setDivider(null);list.setDividerHeight(u.dp(9));list.setSelector(u.touch(Color.TRANSPARENT,18));list.setPadding(0,u.dp(10),0,u.dp(10));list.setClipToPadding(false);list.setAdapter(a);p.addView(list,new LinearLayout.LayoutParams(-1,0,1));adapter=a;}
    private LinearLayout rowCard(){LinearLayout c=u.col();c.setPadding(u.dp(17),u.dp(15),u.dp(17),u.dp(15));c.setBackground(u.bg(u.surface,21));return c;}
    ...
    private void strategyPage(){
        LinearLayout p=listShell();panelTabs(p);EditText search=u.search("搜索策略组或当前节点",nodeSearch);p.addView(search);search.addTextChangedListener(new TextWatcher(){public void beforeTextChanged(CharSequence s,int a,int b,int c){}public void onTextChanged(CharSequence s,int a,int b,int c){nodeSearch=s.toString();if(adapter!=null)adapter.notifyDataSetChanged();}public void afterTextChanged(Editable e){}});LinearLayout tools=u.row();tools.setPadding(0,u.dp(7),0,0);tools.addView(u.text("来自 Mihomo 实际 Clash API",11.5f,u.muted,false),new LinearLayout.LayoutParams(0,-2,1));tools.addView(u.chip("刷新",false,this::poll));p.addView(tools);addList(p,new GroupAdapter());
    }
    ...
    private final class GroupAdapter extends BaseAdapter{
        ...
        public View getView(int pos,View old,ViewGroup parent){if(names.isEmpty())return empty(running?"没有可显示的策略组":"代理尚未运行",running?"当前配置没有 Selector / URLTest 组。":"启动代理后，这里会直接读取 Mihomo 策略组。");String name=names.get(pos);JSONObject g=groups.optJSONObject(name);LinearLayout c=rowCard();LinearLayout h=u.row();h.addView(u.text(name,15.5f,u.text,true),new LinearLayout.LayoutParams(0,-2,1));h.addView(u.text(g.optString("type","Group"),10.5f,u.muted,false));c.addView(h);u.gap(c,7);c.addView(u.text(g.optString("now","未选择"),13,u.accent,true));u.gap(c,5);JSONArray all=g.optJSONArray("all");c.addView(u.text((all==null?0:all.length())+" 个节点 · 点击选择与测速",11.5f,u.muted,false));c.setOnClickListener(v->picker(name));return c;}
    }
    ...
    private void tools(LinearLayout body){
        LinearLayout runtime=u.card(body);runtime.addView(u.text("运行工具",16,u.text,true));u.action(runtime,"activity","运行日志","核心启动 · Root · 透明代理规则",this::showLog);u.action(runtime,"rules","最终启动配置","查看实际交给核心的 startup-config",this::showStartup);
        LinearLayout manage=u.card(body);manage.addView(u.text("管理",16,u.text,true));u.action(manage,"folder","核心与配置","核心导入 · 配置选择 · 自动覆写",()->startActivity(new Intent(this,RootTproxyActivity.class)));u.action(manage,"globe","策略组与节点","节点切换 · 单节点/整组测速",()->{panelTab=0;showPage(1);});u.action(manage,"activity","实时连接","目标 · 规则 · 代理链 · 流量",()->{panelTab=1;showPage(1);});
    }
    ...
    private void settings(LinearLayout body){
        ProxyRuntimeProfile p=profile();ProxyConfigLibrary.Entry e=configs.selected(p.core);LinearLayout current=u.card(body);current.addView(u.text("基础代理配置",16,u.text,true));u.gap(current,8);current.addView(u.text(p.summary()+"\n"+(e==null?"未选择配置":e.name),12,u.muted,false));u.action(current,"settings","进入基础代理配置","核心 · 运行模式 · IPv6 · 自动覆写 · 配置选择",()->startActivity(new Intent(this,RootTproxyActivity.class)));
        LinearLayout note=u.card(body);note.addView(u.text("按 BoxProxy 产品结构重构",14,u.text,true));u.gap(note,7);note.addView(u.text("未接入真实后端的功能不会做成可点击假按钮。后续继续补 TCP/UDP、DNS、QUIC、应用管理、共享网络、绕过规则、网络匹配与 CNIP。",12,u.muted,false));
    }
    ...
    private void stopProxy(){busy=true;setMessage("正在停止并恢复网络规则…");updateVisible();worker.execute(()->{try{root.stop();prefs.edit().putBoolean("rootProxyWanted",false).apply();ui.post(()->{busy=false;running=false;groups=new JSONObject();connections=new JSONObject();setMessage("代理已停止");updateVisible();notifyData();});}catch(Throwable e){String m="停止失败："+safe(e);ui.post(()->{busy=false;setMessage(m);updateVisible();});}});}
    private void poll(){if(polling||closed)return;polling=true;worker.execute(()->{boolean live=false;JSONObject g=new JSONObject(),c=new JSONObject();String error="";try{live=root.status().optBoolean("running",false);if(live){g=controller.proxies();c=controller.connections();}}catch(Exception e){error=safe(e);}boolean r=live;JSONObject gg=g,cc=c;String err=error;ui.post(()->{polling=false;if(closed)return;running=r;groups=gg;connections=cc;if(!err.isEmpty()&&running)setMessage(err);updateVisible();notifyData();});});}
    ...
    private String strategyText(){if(!running)return"启动代理后可选择策略组和节点";List<String> names=groupNames();if(names.isEmpty())return"当前配置没有可显示的策略组";StringBuilder b=new StringBuilder();for(int i=0;i<Math.min(3,names.size());i++){JSONObject g=groups.optJSONObject(names.get(i));if(i>0)b.append('\n');b.append(names.get(i)).append("  →  ").append(g==null?"—":g.optString("now","—"));}if(names.size()>3)b.append("\n还有 ").append(names.size()-3).append(" 个策略组");return b.toString();}
