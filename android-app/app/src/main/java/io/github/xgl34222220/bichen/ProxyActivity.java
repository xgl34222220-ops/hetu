...
public final class ProxyActivity extends Activity {
    ...
    private LinearLayout scrollPage(){ScrollView s=new ScrollView(this);s.setClipToPadding(false);s.setFillViewport(true);u.pad(s,20,5);LinearLayout b=u.col();s.addView(b);content.addView(s,new FrameLayout.LayoutParams(-1,-1));return b;}
    ...
    private void listing(){
        ...
        if(page==2){TextView pause=u.button(frozen?"继续刷新":"暂停刷新",false,()->{frozen=!frozen;choosePage(2);});toolbar.addView(pause,new LinearLayout.LayoutParams(u.dp(102),-2));}b.addView(toolbar);
