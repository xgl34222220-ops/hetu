...
public final class ProxyActivity extends Activity {
    ...
    private TextView title,subtitle,notice,status,detail,configLabel,nodeLabel,upMetric,downMetric,countMetric,connectionHint;
    ...
    private void showPage(int n){
        ...
        page=n;content.removeAllViews();list=null;adapter=null;status=null;detail=null;power=null;upMetric=null;downMetric=null;countMetric=null;configLabel=null;nodeLabel=null;connectionHint=null;
    ...
    private void updateSubscription(String input){task(()->{String expected=store.revision();String address=input.isEmpty()?store.subscription():input;if(address.isEmpty())throw new IOException("尚未保存订阅，请先添加订阅");String yaml=ProxyStore.fetch(address);boolean changed=store.saveIfUnchanged(yaml,address,expected);return changed?"订阅已更新，下次连接使用新配置":"订阅内容无变化，原回滚点已保留";});}
