...
internal class ProxyRuntimeInspector(context: Context) {
    ...
    suspend fun adblockRuntimeStats(): AdblockRuntimeStats = withContext(Dispatchers.IO) {
        val command = "grep -Ei 'bichen-adblock|RuleSet/bichen-adblock' /data/adb/bichen/proxy/run/core.log 2>/dev/null | tail -n 2000 || true"
        ...
        val matched = result.output.lineSequence()
            .filter { it.contains("bichen-adblock", ignoreCase = true) }
            .toList()
