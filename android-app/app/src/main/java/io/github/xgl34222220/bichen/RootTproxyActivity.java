...
/** Single mature proxy control surface: status first, then grouped settings. */
public final class RootTproxyActivity extends Activity {
    ...
    private void build(){
        ...
        LinearLayout titles=u.col();titles.setPadding(u.dp(10),0,0,0);titles.addView(u.text("代理",27,u.text,true));u.gap(titles,5);titles.addView(u.text("基础代理配置 · Root 透明代理",12,u.muted,false));top.addView(titles,new LinearLayout.LayoutParams(0,-2,1));body.addView(top);u.gap(body,13);

        LinearLayout hero=u.card(body);hero.setPadding(u.dp(18),u.dp(17),u.dp(18),u.dp(17));
        ...
        coreValue=settingRow(basic,"核心","",this::chooseCore);separator(basic);modeValue=settingRow(basic,"运行模式","",this::chooseMode);

        section(body,"网络");
        LinearLayout network=u.card(body);network.setPadding(u.dp(17),u.dp(4),u.dp(17),u.dp(4));
        ipv6Value=settingRow(network,"IPv6","",this::chooseIpv6);separator(network);overwriteValue=settingRow(network,"自动覆写","",this::chooseOverwrite);

        section(body,"配置");
        LinearLayout cfg=u.card(body);cfg.setPadding(u.dp(17),u.dp(4),u.dp(17),u.dp(4));
        configValue=settingRow(cfg,"当前配置","尚未选择",this::chooseConfig);separator(cfg);u.action(cfg,"folder","导入新配置","保留源配置，启动时生成独立运行副本",this::importConfig);
    ...
    private String ipv6Label(ProxyRuntimeProfile.Ipv6 v){switch(v){case BYPASS:return"不进核心";case DISABLE:return"禁用系统 IPv6";default:return"启用";}}

    private void toggleService(){if(busy)return;if(running)stopSelectedMode();else startSelectedMode();}
    private void setBusyState(boolean starting,String stage){
        busy=true;stateValue.setText(starting?"● 正在启动":"● 正在停止");stateValue.setTextColor(u.accent);stateMeta.setText(profile().summary());
        actionButton.setEnabled(false);actionButton.setAlpha(.62f);actionButton.setText(starting?"正在启动…":"正在停止…");showFeedback(stage);
    }
    private void postStage(String stage){ui.post(()->{if(!destroyed&&busy)showFeedback(stage);});}
    private void finishBusy(boolean nowRunning,String note){
        busy=false;running=nowRunning;actionButton.setEnabled(true);actionButton.setAlpha(1f);actionButton.setText(nowRunning?"停止代理":"启动代理");
        stateValue.setText(nowRunning?"● 运行中":"● 未运行");stateValue.setTextColor(nowRunning?u.accent:u.text);stateMeta.setText(profile().summary()+(nowRunning?" · 已接管流量":""));
        if(!TextUtils.isEmpty(note))showFeedback(note);
    }
    ...
    private void refresh(){
        ...
        if(busy)return;
        if(p.mode==ProxyRuntimeProfile.Mode.TUN){running=MihomoVpnService.running;finishBusy(running,running?"":"");return;}
