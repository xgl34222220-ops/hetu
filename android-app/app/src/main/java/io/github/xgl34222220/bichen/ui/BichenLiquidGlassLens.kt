...
/**
 * Rounded-rectangle refraction lens used only by the floating dock.
 * Ported from LuoShu's working dock implementation so icons/text stay outside
 * the shader layer and OEM compositors cannot turn them into white rectangles.
 */
internal fun BackdropEffectScope.bichenLiquidGlassLens(
    ...
    val shader = if (dispersion) REFRACTION_DISPERSION_SHADER else REFRACTION_SHADER
    val key = if (dispersion) "BichenLiquidLensDispersion" else "BichenLiquidLens"
    val scale = downscaleFactor.coerceAtLeast(1).toFloat()
    val scaledRadii = FloatArray(radii.size) { radii[it] / scale }
    runtimeShaderEffect(key = key, shaderString = shader, uniformShaderName = "content") {
