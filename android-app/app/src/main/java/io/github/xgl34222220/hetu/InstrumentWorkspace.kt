...
@Composable
internal fun InstrumentProgress(value: Float, modifier: Modifier = Modifier, accent: Color = HetuMicroCrystal.KleinBlue) {
    val fraction = value.takeIf { it.isFinite() }?.coerceIn(0f, 1f) ?: 0f
    val dark = MaterialTheme.colorScheme.background.luminance() < .5f
    val track = if (dark) Color.White.copy(alpha = .11f) else Color(0xFFE8EEF7)
    Box(
        modifier
            .fillMaxWidth()
            .height(5.dp)
        ...
        if (fraction > 0f) {
            Box(
                Modifier
                    .fillMaxWidth(fraction.coerceAtLeast(.01f))
                    .fillMaxHeight()
                    .background(
                        Brush.horizontalGradient(
                            listOf(accent, Color(0xFF38BDF8)),
                        ),
                        CircleShape,
                    ),
            )
