package io.github.xgl34222220.hetu

import android.content.Context

/** Fast runtime truth used by Compose surfaces without treating a slow full status probe as "stopped". */
internal object ProxyStatusBridge {
    fun rootProxyRunning(context: Context): Boolean {
        val app = context.applicationContext
        val prefs = app.getSharedPreferences("hetu", Context.MODE_PRIVATE)
        return try {
            val command = """
                P=$(cat /data/adb/hetu/run/core.pid 2>/dev/null || echo 0)
                case "${'" in ''|*[!0-9]*) P=0;; esac
                if [ "$P" -gt 0 ] && kill -0 "$P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" in ''|*[!0-9]*) P=0;; esac
                if [ "${'" -gt 0 ] && kill -0 "$P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" in ''|*[!0-9]*) P=0;; esac
                if [ "${'" -gt 0 ] && kill -0 "$P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" in ''|*[!0-9]*) P=0;; esac
                if [ "${'" -gt 0 ] && kill -0 "$P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" in ''|*[!0-9]*) P=0;; esac
                if [ "${'" -gt 0 ] && kill -0 "$P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" in ''|*[!0-9]*) P=0;; esac
                if [ "${'" -gt 0 ] && kill -0 "$P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" in ''|*[!0-9]*) P=0;; esac
                if [ "${'" -gt 0 ] && kill -0 "$P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" in ''|*[!0-9]*) P=0;; esac
                if [ "${'" -gt 0 ] && kill -0 "$P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" in ''|*[!0-9]*) P=0;; esac
                if [ "${'" -gt 0 ] && kill -0 "$P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" in ''|*[!0-9]*) P=0;; esac
                if [ "${'" -gt 0 ] && kill -0 "$P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" in ''|*[!0-9]*) P=0;; esac
                if [ "${'" -gt 0 ] && kill -0 "$P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" -gt 0 ] && kill -0 "${'" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/$P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "$CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P" >/dev/null 2>&1; then
                  CMD=$(tr '\000' ' ' < "/proc/${'/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/$P/exe" 2>/dev/null || true)
                  case "${' ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "$CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/cmdline" 2>/dev/null || true)
                  EXE=$(readlink "/proc/${'/exe" 2>/dev/null || true)
                  case "${' ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "$CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}P/exe" 2>/dev/null || true)
                  case "${' ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD $EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}CMD ${'" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
}EXE" in
                    */data/adb/hetu/bin/core*) printf '1';;
                    *) printf '0';;
                  esac
                else
                  printf '0'
                fi
            """.trimIndent()
            val result = RootBridge.rootShell(app, command, 3_500L)
            if (result.ok()) {
                val running = result.output.trim() == "1"
                prefs.edit().putBoolean("proxyRootRuntimeRunning", running).apply()
                running
            } else {
                prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                    prefs.getBoolean("proxyRootWanted", false)
            }
        } catch (_: Exception) {
            prefs.getBoolean("proxyRootRuntimeRunning", false) &&
                prefs.getBoolean("proxyRootWanted", false)
        }
    }
}
