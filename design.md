...
## test.132：紧凑首页与 zashboard 策略面板

采用已确认的轻玻璃方案：状态和操作合为一张主卡，四项运行数据位于一张分隔面板。保留真实数据、48dp 操作目标、字体完整行高和底栏安全距离。320dp 默认字号仍可显示双列策略组；系统大字根据可用宽度改为单列。

策略交互参考 zashboard 官方移动端（MIT，https://github.com/Zephyruso/zashboard ，参考提交 b83a63aeba1a08be62ee4d6228874e1229a12d1b），以原生 Compose 重新实现，没有复制 Vue 代码。默认展开独立节点面板，保留用户明确选择的页内展开偏好。紧凑组卡片的名称与当前节点允许最多两行省略，语义保留全文；节点面板显示当前选择、搜索、排序、测速通过筛选、列表/双列、子策略进入/返回、单节点及整组测速。节点名称长按可展开全文。此预览规则取代 test.131 在折叠组卡片强制显示无限行的规则，展开后的名称仍需完整可读。

节点列表按需组合，失败测量显示超时并从“测速通过”筛选移除，批量测速最多并行 6 个并正确传播取消。选择请求进行中禁用重复选择，成功后更新选中状态，失败显示原因。主题、动效开关继续生效。不得更改受保护的网络运行文件或声称已验证真机微信消息。
