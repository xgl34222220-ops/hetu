...
TPROXY 是辟尘代理模式的 Root 主路径。它不占用 Android VPN 槽位，并保留用户导入配置里的 `listeners`、`tproxy-port`、eBPF、`interface-name`、`routing-mark` 等 Root/Mihomo 配置。

## 运行边界

- Mihomo 以 Root 进程运行，而不是普通 App UID。
- App 只负责配置、启动/停止、状态和日志。
- TPROXY 防火墙链使用独立 `BICHEN_*` 名称，停止/异常恢复时只删除自己的链和策略路由。
- 不修改用户原 YAML；运行时复制到 `/data/adb/bichen/proxy/config.yaml`。
