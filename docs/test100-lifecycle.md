...
- Missing network-repair history is no longer treated as an attempt at boot time zero. First repair may proceed even within the first 30 seconds of boot. The 30-second interval still applies between actual attempts, with a second check under the lock. The full existing 53-check model suite also passes with HETU_TEST_UPTIME=5.
