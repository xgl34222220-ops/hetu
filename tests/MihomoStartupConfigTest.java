...
public final class MihomoStartupConfigTest {
 ...
 public static void main(String[] args)throws Exception{
  ...
  check(t.yaml.contains("listeners:")&&t.yaml.contains("name: hetu-egress-probe")&&t.yaml.contains("type: http")&&t.yaml.contains("listen: 127.0.0.1")&&!t.yaml.contains("type: ebpf")&&!t.yaml.contains("old-tproxy"),"source listeners replaced by loopback-only egress probe");
  ...
  check(raw.yaml.contains("name: hetu-egress-probe")&&!raw.yaml.contains("old-tproxy")&&!raw.yaml.contains("type: ebpf")&&!raw.yaml.contains("global-client-fingerprint:")&&!raw.yaml.contains("stack: gvisor"),"overwrite off isolates source listeners and keeps only the loopback egress probe");
  ...
  MihomoStartupConfig.Result flowResult=MihomoStartupConfig.generate(flow,p(ProxyRuntimeProfile.Mode.TPROXY,true));check(flowResult.yaml.contains("name: hetu-egress-probe")&&!flowResult.yaml.contains("name: e")&&!flowResult.yaml.contains("type: ebpf"),"flow-style source listeners replaced by loopback egress probe");
  String indentless="mode: rule\nlisteners:\n- name: e\n  type: ebpf\n  port: 9898\nrules: ['MATCH,DIRECT']\n";
  MihomoStartupConfig.Result indentlessResult=MihomoStartupConfig.generate(indentless,p(ProxyRuntimeProfile.Mode.TPROXY,true));check(indentlessResult.yaml.contains("name: hetu-egress-probe")&&!indentlessResult.yaml.contains("name: e")&&!indentlessResult.yaml.contains("type: ebpf")&&indentlessResult.yaml.contains("rules:"),"indentless source listener removed without eating rules; probe listener added");
