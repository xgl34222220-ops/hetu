...
public final class Smoke extends Instrumentation {
    ...
    @Override public void onStart(){Bundle result=new Bundle();try{
        ...
        check(pkg.equals("io.github.xgl34222220.hetu.preview"),"unchanged preview package");
        ...
        check(module.getName().equals("Hetu-"+info.getString("version")+"-module.zip"),"export module filename matches its actual metadata");check(sha(module).equals(info.getString("sha256")),"embedded/exported module hash identical");
        try(ZipFile z=new ZipFile(module)){check(z.getEntry("module.prop")!=null&&z.getEntry("bin/hetu")!=null,"export contains module root entries");}
        ...
        String[] pages={"河图·测试","应用放行","过滤规则","请求活动"};String[] dockLabels={"首页","应用","规则","活动"};
        ...
        Object scroll=get("scroll");target.getSharedPreferences("hetu",0).edit().putLong("blocked",431).apply();SystemClock.sleep(450);waitForIdleSync();check(scroll==get("scroll"),"counter event updates existing home instead of rebuilding it");
