...
public final class Control extends Instrumentation {
 ...
 private boolean pending()throws Exception{Method m=service.getDeclaredMethod("pendingSettings",SharedPreferences.class,String.class);m.setAccessible(true);return (Boolean)m.invoke(null,c.getSharedPreferences("hetu",0),c.getPackageName());}
 ...
 @Override public void onStart(){Bundle b=new Bundle();try{
  ...
  c.getSharedPreferences("hetu",0).edit().putBoolean("vpnWanted",false).putBoolean("vpnRestoreHosts",false).putBoolean("proxyManageHosts",false).putBoolean("proxyFilter",true).putBoolean("proxyDnsGuard",true).putBoolean("source_hagezi",true).putStringSet("user_allow",Collections.singleton("safe.0.0-02.net")).commit();
  ...
  Class<?> rs=Class.forName(pkg+".RuleStore",true,cl);Object rules=rs.getConstructor(Context.class).newInstance(c);rs.getMethod("reload").invoke(rules);rs.getMethod("changeDomain",String.class,boolean.class,boolean.class,boolean.class).invoke(rules,"ads.hetu.test",false,true,false);
  ...
  startServiceAndWait();
  SharedPreferences prefs=c.getSharedPreferences("hetu",0);Set<String> bypass=new HashSet<>(Arrays.asList("hetu.proxyprobe","hetu.nonexistent.fixture"));
  ...
  rs.getMethod("changeDomain",String.class,boolean.class,boolean.class,boolean.class).invoke(rules,"new.ads.hetu.test",false,true,false);
  ...
  List<?> captured=observations(seen);check(captured.toString().contains("allowed.hetu.test"),"service sampler retains actual independent-UID domain connection");check(!captured.toString().contains("ads.hetu.test"),"rejected requests are not fabricated into active-connection observations");
  ...
  check(((Set<?>)policy("applied")).equals(Collections.singleton("hetu.proxyprobe")),"installed selected app is actually excluded by VPN Builder");
  check(((Set<?>)policy("missing")).equals(Collections.singleton("hetu.nonexistent.fixture")),"missing package is not counted as an applied exclusion");check(!pending(),"uninstalled selection alone does not cause endless reconnect warning");
  ...
  mark("proxy-checks.txt",result.toString());b.putString("stream",result+"HETU_MIHOMO_CONTROL_PASS checks="+checks+"\nNo OEM/Root framework or commercial subscription was tested.\n");finish(Activity.RESULT_OK,b);
 }catch(Throwable e){try{mark("control-error.txt",android.util.Log.getStackTraceString(e));}catch(Throwable ignored){}b.putString("stream",result+"HETU_MIHOMO_CONTROL_FAIL\n"+android.util.Log.getStackTraceString(e));finish(Activity.RESULT_CANCELED,b);}}
}
