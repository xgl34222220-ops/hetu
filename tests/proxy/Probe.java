...
import java.net.*;
import java.io.*;
/** Separate UID. Normal and ad requests use exactly the same network code. */
public final class Probe extends Activity {
 @Override public void onCreate(Bundle b){super.onCreate(b);new Thread(()->{
  boolean allowed=false,blocked=false,adsAllowed=false;String detail="";
  ...
  try{adsAllowed=fetch("http://ads.integration.test/check").equals("BICHEN_PROXY_OK");}catch(IOException expected){blocked=true;}
  sendBroadcast(new Intent("bichen.integration.RESULT").setPackage("io.github.xgl34222220.bichen.preview").putExtra("allowed",allowed).putExtra("blocked",blocked).putExtra("adsAllowed",adsAllowed).putExtra("detail",detail));runOnUiThread(this::finish);
 }).start();}
