...
public final class Probe extends Activity {
 @Override public void onCreate(Bundle b){super.onCreate(b);new Thread(()->{
  ...
  try{allowed=fetch("http://normal.integration.test/check").equals("HETU_PROXY_OK");}catch(Exception e){detail=e.getClass().getSimpleName();}
  try{fetch("http://ads.integration.test/check");}catch(IOException expected){blocked=true;}
  sendBroadcast(new Intent("hetu.integration.RESULT").setPackage("io.github.xgl34222220.hetu.preview").putExtra("allowed",allowed).putExtra("blocked",blocked).putExtra("detail",detail));runOnUiThread(this::finish);
 }).start();}
