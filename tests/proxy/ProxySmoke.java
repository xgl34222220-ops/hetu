...
/** Actual full-tunnel integration with a separate application UID and controlled proxy. */
public final class ProxySmoke extends Instrumentation {
 private int checks;private StringBuilder log=new StringBuilder();private Context context;private Class<?> nativeClass,service;
 ...
 @Override public void onStart(){Bundle result=new Bundle();try{
  ...
  Class<?> storeClass=Class.forName(pkg+".ProxyStore",true,loader);Constructor<?> ctor=storeClass.getDeclaredConstructor(Context.class);ctor.setAccessible(true);Object store=ctor.newInstance(context);
  Method save=storeClass.getDeclaredMethod("save",String.class,String.class);save.setAccessible(true);
  ...
  call(new JSONObject().put("action","connections"));check(true,"actual connection snapshot callable");
  Activity screen=startActivitySync(new Intent(context,Class.forName(pkg+".ProxyActivity",true,loader)).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK));waitForIdleSync();SystemClock.sleep(700);
  check(!screen.isFinishing(),"proxy configuration screen opens");
