...
public final class UiCheck extends Instrumentation {
 ...
 String strings(){final StringBuilder b=new StringBuilder();runOnMainSync(()->collect(a.getWindow().getDecorView(),b));return b.toString();}
 ...
 TextView findTextOnMain(String value){final TextView[] result=new TextView[1];runOnMainSync(()->result[0]=findText(a.getWindow().getDecorView(),value));return result[0];}
 ...
 @Override public void onStart(){Bundle b=new Bundle();try{
  ...
  Class<?> profileClass=Class.forName(c.getPackageName()+".ProxyRuntimeProfile",true,c.getClassLoader());Method load=profileClass.getDeclaredMethod("load",SharedPreferences.class);load.setAccessible(true);Object profile=load.invoke(null,prefs);Object manager=create("RootProxyManager");Object prepared=invoke(manager,"prepare",new Class<?>[]{profileClass},profile);String startup=(String)field(prepared,"startup");
  check(!startup.contains("listeners:")&&!startup.contains("type: ebpf")&&!startup.contains("old-tproxy"),"Root TPROXY startup copy removes source listeners for other runtimes");
  check(!startup.contains("global-client-fingerprint:")&&startup.contains("tproxy-port: 9898"),"Root startup copy removes obsolete global fingerprint and injects selected TPROXY port");
  check(rootYaml.contains("type: ebpf")&&rootYaml.contains("global-client-fingerprint: chrome"),"mode overwrite never edits imported source config");
  Object legacy=create("ProxyStore");boolean tunRejected=false;try{invoke(legacy,"saveIfUnchanged",new Class<?>[]{String.class,String.class,String.class},rootYaml,"",invoke(legacy,"revision",new Class<?>[0]));}catch(InvocationTargetException expected){tunRejected=expected.getCause()!=null;}check(tunRejected,"Android TUN validation stays separate from Root source library");
  for(String appearance:new String[]{"light","dark"}){
   ...
   if("light".equals(appearance)){
    final TextView start=findTextOnMain("启动代理");check(start!=null,"start action can be located on main thread");runOnMainSync(()->start.performClick());SystemClock.sleep(160);String immediate=strings();check(immediate.contains("正在启动")||immediate.contains("启动失败"),"start tap produces immediate visible feedback");SystemClock.sleep(1600);String later=strings();check(later.contains("正在启动")||later.contains("启动失败")||later.contains("需要 Root")||later.contains("Root")||later.contains("运行中"),"start never fails silently");
   }
   shot(appearance+"-proxy-control");runOnMainSync(()->{if(a!=null&&!a.isFinishing())a.finish();});waitForIdleSync();SystemClock.sleep(300);
  }
  prefs.edit().putString("appearance","system").commit();b.putString("stream",output+"BICHEN_PROXY_UI_PASS checks="+checks+"\nMode-aware startup copy strips incompatible listeners; proxy action gives immediate feedback.\n");finish(Activity.RESULT_OK,b);
