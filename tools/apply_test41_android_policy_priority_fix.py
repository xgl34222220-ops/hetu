...
# test.40 placed the Hetu fwmark policy rule at 28700. On Android 16,
# normal network selection rules (including the socket's netId) run before that,
# so marked local OUTPUT traffic can be routed to wlan/rmnet before Hetu's
...
new = '''  # Keep Android VPN/lockdown guards ahead of Hetu, but run before ordinary
...
old_status = '''  printf '{"ok":true,"running":%s,"pid":%s,"mode":"%s","ipv4Rules":%s,"ipv6Rules":%s,"killSwitchActive":%s,"ipv6DisabledByHetu":%s,"watchdog":%s,"recoveredStaleRules":%s,"mark":"%s","table":"%s","appScope":"%s","sharedNetwork":"%s","killSwitchRequested":"%s","log":"%s","configCheckLog":"%s"}\\n' "$STATUS_RUNNING" "$STATUS_PID" "$STATUS_MODE" "$T4" "$T6" "$([ "$K4" = true ] || [ "$K6" = true ] && echo true || echo false)" "$V6OFF" "$WD" "$RECOVERED" "$SM" "$ST" "$SCOPEV" "$SHAREV" "$KILLV" "$LOG" "$CHECKLOG"'''
new_status = '''  SP=$(state_value PREF 2>/dev/null || true)
  printf '{"ok":true,"running":%s,"pid":%s,"mode":"%s","ipv4Rules":%s,"ipv6Rules":%s,"killSwitchActive":%s,"ipv6DisabledByHetu":%s,"watchdog":%s,"recoveredStaleRules":%s,"mark":"%s","table":"%s","pref":"%s","appScope":"%s","sharedNetwork":"%s","killSwitchRequested":"%s","log":"%s","configCheckLog":"%s"}\\n' "$STATUS_RUNNING" "$STATUS_PID" "$STATUS_MODE" "$T4" "$T6" "$([ "$K4" = true ] || [ "$K6" = true ] && echo true || echo false)" "$V6OFF" "$WD" "$RECOVERED" "$SM" "$ST" "$SP" "$SCOPEV" "$SHAREV" "$KILLV" "$LOG" "$CHECKLOG"'''
