#!/usr/bin/env python3
"""Build isolated preview and assert the Root proxy console ships real strategy controls."""
from __future__ import annotations
import hashlib,json,os,re,shutil,subprocess,sys,tempfile,zipfile
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]
BASE='io.github.xgl34222220.bichen';PREVIEW=BASE+'.preview';VERSION='0.4.0-test.12';CODE=412
...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='bichen-preview-') as temp:
  ...
  main_dir=stage/'android-app/app/src/main';manifest=main_dir/'AndroidManifest.xml';value=manifest.read_text();assert f'package="{BASE}"' in value;assert value.count('android.intent.category.LAUNCHER')==1
  ...
  for path in list((main_dir/'java').rglob('*.java'))+list((stage/'tests').glob('*.java')):
   ...
   if path.name=='MainActivity.java':text=text.replace('"辟尘"','"辟尘·测试"').replace('"少一点打扰，多一点清净"','"测试版 · 请先停止旧版保护与自动更新"')
   path.write_text(text)
  proxy=(main_dir/'java/io/github/xgl34222220/bichen/ProxyActivity.java').read_text()
  for required in ('showPage(','策略组','全部测速','实时连接','MihomoControllerClient','controller.select','controller.delay','RootTproxyActivity.class'):assert required in proxy
  startup=(main_dir/'java/io/github/xgl34222220/bichen/MihomoStartupConfig.java').read_text()
  for required in ('removeTopLevelKey(yaml,"listeners")','removeTopLevelScalar(yaml,"global-client-fingerprint")','external-controller: 127.0.0.1:','proxyControllerSecret'):pass
  assert 'external-controller: 127.0.0.1:' in startup and 'CONTROLLER_PORT=19090' in startup
