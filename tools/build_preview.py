...
BASE='io.github.xgl34222220.bichen';PREVIEW=BASE+'.preview';VERSION='0.4.0-test.19';CODE=419
...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='bichen-preview-') as temp:
  ...
  dock=(main_dir/'java/io/github/xgl34222220/bichen/LuoShuDockView.java').read_text();assert 'setCornerRadius(dp(31))' in dock and 'dp(23)' in dock and 'dp(60)' in dock and 'setElevation(dp(18))' in dock and 'OvershootInterpolator' in dock
  skin=(main_dir/'java/io/github/xgl34222220/bichen/BichenApplication.java').read_text();assert 'styleMainDock' in skin and 'lastDockGeometry' in skin and 'system_accent1_500' in skin and '0xfff4f6fa' in skin and 'MainActivity.class.getDeclaredField("nav")' in skin and 'LuoShuDockView' in skin
  ...
  ui=(main_dir/'java/io/github/xgl34222220/bichen/ProxyUi.java').read_text();assert '0xfff4f6fa' in ui and 'system_accent1_' in ui and 'emphasizedCard' in ui and 'setElevation(dp(1))' in ui and 'LuoShuSurfaceDrawable' in ui
  ...
  for name in ('ProxyCoreStore.java','ProxyConfigLibrary.java','ProxyRuntimeProfile.java','RootProxyManager.java','MihomoControllerClient.java','LuoShuDockView.java','LuoShuSurfaceDrawable.java','BichenApplication.java'):assert (main_dir/'java/io/github/xgl34222220/bichen'/name).is_file()
  ...
  with zipfile.ZipFile(unsigned) as apk:
   ...
   for name in ('ProxyActivity','RootTproxyActivity','RootProxyManager','ProxyRuntimeProfile','ProxyCoreStore','ProxyConfigLibrary','MihomoControllerClient','LuoShuDockView','LuoShuSurfaceDrawable','BichenApplication'):assert f'Lio/github/xgl34222220/bichen/preview/{name};'.encode() in dex
