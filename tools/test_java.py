...
PROTOCOL_TESTS = ('DnsPacketTest', 'DnsCacheTest', 'DnsUpstreamTest', 'DnsResponseFilterTest', 'RuleProfilesTest', 'NetworkEpochTest', 'RuleUpdateGateTest', 'ModuleArchiveTest', 'ProtectionStateTest')
...
def main():
    ...
    if args.protocol_only:
        ...
        sources += [package / (name + '.java') for name in ('DnsPacket', 'DnsCache', 'DnsUpstream', 'DnsResponseFilter', 'RuleProfiles', 'NetworkEpoch', 'RuleUpdateGate', 'ModuleArchive', 'ProtectionState')]
        cp = str(dest)
    else:
        _, android = sdk_paths()
