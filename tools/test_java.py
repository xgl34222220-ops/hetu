...
PROTOCOL_TESTS = ('DnsPacketTest', 'DnsCacheTest', 'DnsUpstreamTest', 'DnsResponseFilterTest', 'RuleProfilesTest')
...
def main():
    ...
    if args.protocol_only:
        ...
        sources += [package / (name + '.java') for name in ('DnsPacket', 'DnsCache', 'DnsUpstream', 'DnsResponseFilter', 'RuleProfiles')]
        cp = str(dest)
    else:
        _, android = sdk_paths()
