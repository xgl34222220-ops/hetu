...
PKG='io.github.xgl34222220.hetu.preview';DIR='/data/user/0/'+PKG+'/files/'
...
class Socks(socketserver.BaseRequestHandler):
 def handle(self):
  ...
  try:
   ...
   if request.startswith(b'GET /hetu-e2e '):
    s.sendall(b'HTTP/1.0 200 OK\r\nContent-Length: 16\r\n\r\nHETU_PROXY_E2E!');time.sleep(3)
...
build('control','hetu.mihomotest',PKG,'Control');build('probe','hetu.proxyprobe','hetu.proxyprobe','Probe')
...
control=subprocess.Popen(['adb','shell','am','instrument','-w','hetu.mihomotest/.Control'],stdout=control_output,stderr=subprocess.STDOUT)
def wait_for(name):
 for i in range(180):
...
def probe(mode,suffix):
 p=run('adb','shell','am','instrument','-w','-e','mode',mode,'hetu.proxyprobe/.Probe',capture_output=True,text=True,timeout=75);(O/(suffix+'-probe.txt')).write_text(p.stdout+p.stderr);print(p.stdout);assert 'HETU_MIHOMO_PROBE_PASS' in p.stdout and 'HETU_MIHOMO_PROBE_FAIL' not in p.stdout
try:
 ...
 text=(O/'control-results.txt').read_text();print(text);assert 'HETU_MIHOMO_CONTROL_PASS' in text and 'CONTROL_FAIL' not in text
 assert any(x['host']=='198.51.100.7' for x in received), 'proxy did not receive real TUN request'
 assert any(x['host']=='allowed.hetu.test' for x in received), 'DNS domain mapping not observed'
 assert any(x['host']=='safe.0.0-02.net' for x in received), 'PASS whitelist did not preserve original proxy routing'
 assert not any(x['host']=='ads.hetu.test' for x in received), 'exact blocked domain reached upstream proxy'
 ...
 print('HETU_MIHOMO_E2E_PASS: real TUN, SOCKS routing, DNS, exact/suffix ad rejection, PASS whitelist routing, encrypted DNS domain/853 guard, pure-IP TLS SNI guard, stop, restart, app bypass, pending settings, reinclude; independent UID')
finally:
