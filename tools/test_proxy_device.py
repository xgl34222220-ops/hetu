...
subprocess.run(['adb','uninstall','io.github.xgl34222220.bichen.preview'],capture_output=True)
meta=json.loads((R/'out/preview/build-info.json').read_text());sign(R/f'out/preview/Bichen-{meta["versionName"]}-unsigned.apk',O/'app.apk');run('adb','install',O/'app.apk')
for kind,pkg,cls in [('test','bichen.proxytest','ProxySmoke'),('probe','bichen.probe','Probe')]:
...
for attempt in range(20):
 ...
 time.sleep(.25)
run('adb','pull','/data/user/0/io.github.xgl34222220.bichen.preview/files/mihomo-screen.png',O/'mihomo-screen.png',timeout=20)
